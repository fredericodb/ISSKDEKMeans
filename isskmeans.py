...
class ISSKMeans(BaseEstimator):
    ...
    def __init__(self, nk=2, n=2, w=0, dt='euclidean', cini='quantile', nit=100, thd=0, alpha=0.75, mo=1, ver=2):
        self.clusters = 0
        self.cf_n = 0
        self.cf_ls = 0
        self.cf_ss = 0
    ...
    def fit(self, X, T):
        ...
        print('ISSKMeans(nk=%d, n=%d, dt=%s, cini=%s, nit=%d, thd=%f, alpha=%f, mo=%d)' % (self.nk, self.n, self.dt, self.cini, self.nit, self.thd, self.alpha, self.mo))
        if self.ver == 0:
    ...
    def fit_unlabeled(self, xi, wd=0, vector=False):
        ...
        if nnc.size == 1:
            ...
            nncluster = nnc[0]
            if self.cf_class[nnc[0]] == self.cf_class[nnc[1]]:
                w1 = 1
                w2 = 0
            else:
        ...
        if ncls != -1:
            ...
            if wd == 1: # linear weight
                if w1 != 0:
                    ncls = nnc[0]
                    self.cf_n[ncls] = self.cf_n[ncls] + w1
                    self.cf_ls[ncls,:] = self.cf_ls[ncls,:] + w1 * xi
                    self.cf_ss[ncls,:] = self.cf_ss[ncls,:] + w1 * xi ** 2
                    self.cf_time[ncls] = 0
                    self.clusters[ncls,:] = self.cf_ls[ncls,:] / self.cf_n[ncls]
                ...
                if w2 != 0:
                    ncls = nnc[1]
                    self.cf_n[ncls] = self.cf_n[ncls] + w2
                    self.cf_ls[ncls,:] = self.cf_ls[ncls,:] + w2 * xi
                    self.cf_ss[ncls,:] = self.cf_ss[ncls,:] + w2 * xi ** 2
                    self.cf_time[ncls] = 0
                    self.clusters[ncls,:] = self.cf_ls[ncls,:] / self.cf_n[ncls]
...
class ISSKMeansEnsemble(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        for i in range(0, self.nkm):
            self.kms.append(ISSKMeans(self.nk, sum(self.fsel[i, :]), self.w[:, self.fsel[i, :]], self.dt, self.cini, self.nit, self.thd, self.alpha, self.mo, self.ver))
