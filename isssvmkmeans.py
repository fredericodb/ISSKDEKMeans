...
class ISSSVMKMeans(BaseEstimator):
    ...
    def fit(self, X, T):
        ...
        if self.plug:
            ...
            for i in range(0, nk):
                ...
                iyc = iyc[0]
                if iyc.shape[0] > 0:
                    # find samples of majority class in cluster
                    iycc = (T[iyc] == self.cf_class[i]).nonzero()
                    iycc = iycc[0]
                    kde = svm.OneClassSVM(nu=0.1, kernel="rbf", gamma=0.1)
                    kde.fit(X[iyc[iycc]])
    ...
    def fit_unlabeled(self, xi, wd=0, vector=False):
        ...
        if self.plug:
            ...
            for i in range(self.nk):
                itsin = self.kde_clusterclass[i].predict(xi.reshape(1, -1))
                if (itsin == 1) and ((nnc[0] == i) or (nnc[1] == i)):
                    nncluster_kde = i
                    break
            if nncluster_kde == -1:
                nncluster = -1
